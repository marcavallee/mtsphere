...
def HSSphere_Ker(lmbda,sxlyr,nlyr,thk,dpthl,zs,yhat,zhat):
    ...
    if nlyr == 0:
        ...
        Fn = np.zeros((2,2,1))
        return An, Fn
    An = np.zeros((2, 2, nlyr + 1), dtype=complex)
    Fn = np.zeros((2, 2, nlyr + 1), dtype=complex)
    At = np.zeros((nlyr+1, 2),dtype=complex)
    Ft = np.zeros((nlyr+1, 2),dtype=complex)
    ...
    if sxlyr == 0:
        ...
        if ep.real < EXP_TOL:
            ...
            Ej = 0
        Fn[1,0,0] = Ej * ( adm[0] - admhat[1] ) / ( adm[0] + admhat[1] )
        An[1,0,0] = Ej * ( imp[0] - imphat[1] ) / ( imp[0] + imphat[1] )
        Ft[1,0] = Fn[1,0,0] + Ej
        At[1,0] = An[1,0,0] + Ej
    elif sxlyr == nlyr:
        ...
        if abs ( adm[nlyr] - admhat[nlyr] ) > 0:
            Fn[0,1,nlyr] = Ej * ( adm[nlyr] + admhat[nlyr] ) / ( adm[nlyr] - admhat[nlyr] )
        if abs ( imp[nlyr] - imphat[nlyr] ) > 0:
            An[0,1,nlyr] = Ej * ( imp[nlyr] + imphat[nlyr] ) / ( imp[nlyr] - imphat[nlyr] )
        Ft[nlyr,1] = Fn[0,1,nlyr] + Ej
        At[nlyr,1] = An[0,1,nlyr] + Ej
    else:
        ...
        An[1,1,sxlyr] =   ( imphat[sxlyr+1] - imp[sxlyr] ) * ( imphat[sxlyr] + imp[sxlyr] ) * E2h * Es1 / Aden
        Ft[sxlyr,0] = Fn[0,0,sxlyr] + Fn[1,0,sxlyr]
        Ft[sxlyr,1] = Fn[0,1,sxlyr] + Fn[1,1,sxlyr] + Es1
        At[sxlyr,0] = An[0,0,sxlyr] + An[1,0,sxlyr]
        At[sxlyr,1] = An[0,1,sxlyr] + An[1,1,sxlyr] + Es1
        Ft[sxlyr+1,0] = Fn[0,0,sxlyr] * Emh + Fn[1,0,sxlyr] * Eph + Es2
        Ft[sxlyr+1,1] = Fn[0,1,sxlyr] * Emh + Fn[1,1,sxlyr] * Eph
        At[sxlyr+1,0] = An[0,0,sxlyr] * Emh + An[1,0,sxlyr] * Eph + Es2
        At[sxlyr+1,1] = An[0,1,sxlyr] * Emh + An[1,1,sxlyr] * Eph
